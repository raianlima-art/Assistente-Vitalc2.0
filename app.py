...
# -----------------------------------------------------------------------------
# 1. CARREGAMENTO DAS VARIÁVEIS DE AMBIENTE E SEGREDOS
# -----------------------------------------------------------------------------
...
if not GOOGLE_DRIVE_FOLDER_ID and "gdrive" in st.secrets and "folder_id" in st.secrets["gdrive"]:
    GOOGLE_DRIVE_FOLDER_ID = st.secrets["gdrive"]["folder_id"]
if not GOOGLE_DRIVE_FOLDER_ID:
    GOOGLE_DRIVE_FOLDER_ID = "1cGkRQQVSA8sNCMah-kUXVAQWDILs_grh"
...
# -----------------------------------------------------------------------------
# 4. FUNÇÕES AUXILIARES E OTIMIZADAS COM CACHE
# -----------------------------------------------------------------------------
...
def limpar_texto_appsheet(texto):
    if not texto:
        return ""
    
    txt = str(texto)
    
    def formatar_link_reportlab(match):
        url = match.group(1)
        rotulo = match.group(2)
        rotulo_limpo = re.sub(r'<[^>]+>', '', rotulo).strip()
        rotulo_limpo = rotulo_limpo.replace("■", "").strip()
        return f'<a href="{url}"><font color="#0284c7"><u>{rotulo_limpo}</u></font></a>'

    if "<a " in txt.lower():
        txt = re.sub(r'<a\s+[^>]*href=["\']([^"\']+)["\'][^>]*>(.*?)</a>', formatar_link_reportlab, txt, flags=re.IGNORECASE)
        txt = re.sub(r'<(?!/?(b|i|u|font|a)\b)[^>]+>', '', txt)
    else:
        txt = html.escape(txt)
...
@st.cache_data(ttl=600, show_spinner=False)
def obter_lista_tecnicos():
    lista = ["Raian", "Allan", "Kevin"]
    if supabase:
        try:
            res_tec = supabase.table("tecnicos").select("nome").execute()
            if res_tec.data:
                return sorted(list(set(row["nome"] for row in res_tec.data)))
        except Exception:
            pass
    return lista
    ...
    try:
        ...
        if os.path.exists("credentials.json"):
            try:
                ...
                print(f"Erro ao autenticar com credentials.json: {e_file}")

        creds_raw = get_secret("GOOGLE_DRIVE_CREDENTIALS")
        if not creds_raw and "gdrive" in st.secrets and "credentials" in st.secrets["gdrive"]:
            creds_raw = st.secrets["gdrive"]["credentials"]
        ...
        if isinstance(creds_raw, dict):
            creds_json = creds_raw.copy()
        elif hasattr(creds_raw, "to_dict"):
            creds_json = creds_raw.to_dict()
        elif isinstance(creds_raw, str):
            s_clean = creds_raw.strip()
            try:
                creds_json = json.loads(s_clean)
            except Exception:
                try:
                    decoded_bytes = base64.b64decode(s_clean)
...
def salvar_nf_no_drive(file_bytes, nome_arquivo, mime_type="application/pdf", as_google_doc=False, nome_subpasta=None):
    try:
        ...
        folder_root = GOOGLE_DRIVE_FOLDER_ID or "1cGkRQQVSA8sNCMah-kUXVAQWDILs_grh"

        if not nome_subpasta:
            data_hoje = datetime.datetime.now().strftime("%d-%m-%Y")
            nome_subpasta = ["Relatórios IA", "Backup", data_hoje]

        pasta_destino_id = obter_ou_criar_caminho_pastas(service, folder_root, nome_subpasta)
...
def gerar_pdf_bonificacao_mensal(tecnico, mes_ano_str, soma_pontos, f_meta, f_retrabalho, val_bonus, os_aprovadas):
    ...
    info_data = [
        [Paragraph("Técnico:", lbl_style), Paragraph(limpar_texto_appsheet(nome_tec_completo), val_style)],
        [Paragraph("Mês/Ano de Referência:", lbl_style), Paragraph(limpar_texto_appsheet(mes_ano_str), val_style)],
    ...
    for item in os_aprovadas:
        ...
        os_data.append([
            ...
            Paragraph(limpar_texto_appsheet(item.get("numero_os", "")), td_bold),
            Paragraph(limpar_texto_appsheet(item.get("tipo_servico", "")), td_style),
            Paragraph(limpar_texto_appsheet(item.get("complexidade", "")), td_style),
            Paragraph(limpar_texto_appsheet(item.get("tipo_os", "")), td_style),
    ...
    sig_data = [
        [
            Paragraph(f"___________________________________<br/><b>Assinatura do Colaborador</b><br/>{limpar_texto_appsheet(nome_tec_completo)}", sig_style),
            Paragraph(f"___________________________________<br/><b>Assinatura do Gestor</b><br/>{limpar_texto_appsheet(nome_gestor_completo)}", sig_style)
        ]
    ]
...
def gerar_pdf_controle_compras(resp_cot_hist, mes_referencia="Todos", ano_referencia="2026"):
    ...
    # --- GERAR GRÁFICO VISUAL DENTRO DO PDF ---
    meses_grafico = []
    pct_grafico = []
    ...
    for m_idx in range(1, 13):
        ...
        if med > 0 or econ > 0:
            meses_grafico.append(MESES_NOME[m_idx - 1])
            pct_grafico.append(pct)

    if pct_grafico:
        fig_plt, ax = plt.subplots(figsize=(10, 2.8), dpi=200)
        x_pos = np.arange(len(meses_grafico))
        
        bars = ax.bar(x_pos, pct_grafico, color='#c6d9f1', edgecolor='#002060', linewidth=1.2, width=0.55)
        ...
        if len(pct_grafico) > 1:
            z = np.polyfit(x_pos, pct_grafico, 1)
            p = np.poly1d(z)
            ax.plot(x_pos, p(x_pos), color="red", linewidth=2)

        ax.set_ylabel('% Economia', fontsize=8, color='#475569', fontweight='bold')
        ax.set_xticks(x_pos)
        ax.set_xticklabels(meses_grafico, fontsize=8)
        ax.yaxis.set_major_formatter('{x:.0f}%')
        ax.set_ylim(0, max(max(pct_grafico) * 1.35, 20))
        ...
        elements.append(Image(img_buf, width=780, height=210))
    ...
    for m_idx in range(1, 13):
        ...
        if 0 < pct < 10.0:
            pct_cell = Paragraph(f"<font color='#dc2626'><b>{pct:.2f}%</b></font>", td_res_bold)
        else:
            pct_cell = Paragraph(f"{pct:.2f}%", td_res_bold)
    ...
    if cotacoes_filtradas_mes:
        ...
        for row_idx, item in enumerate(cotacoes_filtradas_mes, start=1):
            ...
            prod = limpar_texto_appsheet(item.get("produto", "N/A"))
            forn_a = limpar_texto_appsheet(item.get("fornecedor_a", "N/A"))
            forn_b = limpar_texto_appsheet(item.get("fornecedor_b", "N/A"))
            forn_c = limpar_texto_appsheet(item.get("fornecedor_c", "N/A"))
...
def gerar_pdf_cotacao_individual(desc, qtd, ref, solic, motivo, opcoes_ordenadas, vencedor):
    ...
    desc_clean = limpar_texto_appsheet(desc)
    ref_clean = limpar_texto_appsheet(ref)
    solic_clean = limpar_texto_appsheet(solic)
    motivo_clean = limpar_texto_appsheet(motivo)

    info_data = [
        [Paragraph("Item / Produto:", lbl_style), Paragraph(desc_clean, val_style)],
        [Paragraph("Quantidade:", lbl_style), Paragraph(f"{qtd} un.", val_style)],
        [Paragraph("Referência / Modelo:", lbl_style), Paragraph(ref_clean, val_style)],
        [Paragraph("Solicitante:", lbl_style), Paragraph(solic_clean, val_style)],
        [Paragraph("Motivo da Compra:", lbl_style), Paragraph(motivo_clean, val_style)],
    ...
    for idx, op in enumerate(opcoes_ordenadas, 1):
        ...
        nome_fornecedor_clean = limpar_texto_appsheet(op["nome"])

        row = [
            Paragraph(nome_fornecedor_clean, td_bold if is_aprovado else td_style),
    ...
    vencedor_nome_clean = limpar_texto_appsheet(vencedor['nome'])
    ...
    venc_box_data = [
        ...
        [Paragraph(f"<b>{vencedor_nome_clean}</b> — Valor Total Aprovado: <b>R$ {formar_real(vencedor['total'])}</b>", ParagraphStyle("VD", fontSize=9, leading=12, fontName="Helvetica", textColor=colors.HexColor("#14532d")))],
    ]
...
def enviar_email_notificacao(descricao, link, referencia, quantidade, motivo, solicitante, urgencia="Média", tipo_compra="Nacional", id_manutencao=None, compativel=None, encapsulamento=None, custo_estimado=None, link_adicional=None, datasheet=None):
    ...
    msg["Subject"] = f"🚨 Nova Solicitação de Compra ({tipo_compra}) - {solicitante} ({urgencia})"
    ...
    <ul>
        ...
        <li><strong>Origem da Compra:</strong> {tipo_compra}</li>
...
def registrar_solicitacao_compra(descricao, link, referencia, quantidade, motivo, solicitante, urgencia="Média", tipo_compra="Nacional", id_manutencao=None, compativel=None, encapsulamento=None, custo_estimado=None, link_adicional=None, datasheet=None):
    if supabase:
        payload = {
        ...
        try:
            ...
            extra_text = f" | Origem: {tipo_compra} | ID Manut: {id_manutencao} | Compativel: {compativel} | Encapsulamento: {encapsulamento} | Custo Est: {custo_estimado}"
            payload_fallback = {
        ...
        enviar_email_notificacao(descricao, link, referencia, quantidade, motivo, solicitante, urgencia, tipo_compra, id_manutencao, compativel, encapsulamento, custo_estimado, link_adicional, datasheet)
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "tipo_compra": {
                        ...
                        "description": "Origem da compra: Nacional ou Internacional"
                    }
                },
                "required": ["descricao", "quantidade", "motivo", "solicitante", "urgencia", "tipo_compra"]
            }
        }
    },
    {
    ...
    with aba_login_user:
        ...
        if "Sem Senha" in tipo_acesso:
            ...
            with st.form("form_identificacao_tecnico"):
                lista_tecnicos = obter_lista_tecnicos()
...
if st.session_state.is_adm:
    aba_chat, aba_gestao, aba_inter, aba_gestor, aba_ferramentas = st.tabs([
        "💬 Assistente",
        "📋 Compras",
        "🌍 Importações",
        "🏆 Auditoria",
            ...
            if filtro_est_status == "Selecione para filtrar...":
                ...
                if not dados_nf:
                    ...
                    for item in dados_nf:
                        ...
                        link_invoice = item.get("link_invoice")
                        link_dir = item.get("link_dir")
                        num_pedido = item.get("numero_pedido", "N/A")
                        status_atual = item.get("status", "Pendente")
                        tipo_c_item = item.get("tipo_compra", "Nacional")
                        ...
                        badge_origem = "🇧🇷 Nacional" if tipo_c_item == "Nacional" else "✈️ Internacional"
                        ...
                        if link_nf:
                            botoes_docs += f'<a href="{link_nf}" target="_blank" style="background: #eff6ff; color: #1d4ed8; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 0.8rem; border: 1px solid #bfdbfe; margin-right: 6px;">📄 Nota Fiscal ↗</a>'
                        if link_invoice:
                            botoes_docs += f'<a href="{link_invoice}" target="_blank" style="background: #faf5ff; color: #7e22ce; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 0.8rem; border: 1px solid #e9d5ff; margin-right: 6px;">📄 Invoice ↗</a>'
                        if link_dir:
                            botoes_docs += f'<a href="{link_dir}" target="_blank" style="background: #faf5ff; color: #7e22ce; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 0.8rem; border: 1px solid #e9d5ff; margin-right: 6px;">📄 DIR ↗</a>'
                        ...
                        card_html = (
                            ...
                            f'<span style="font-weight: 600; font-size: 0.95rem; color: #0f172a;">'
                            f'📦 {desc} <span style="font-size: 0.85rem; color: #64748b; font-weight: 400;">({qtd} un.)</span>'
                            f'<span style="background: #e2e8f0; color: #334155; padding: 2px 6px; border-radius: 4px; font-size: 0.725rem; font-weight: 600; margin-left: 6px;">{badge_origem}</span>'
                            f'</span>'
            ...
            if not supabase:
                ...
                if not dados_os:
                    ...
                    if tec_sel != "Todos os Técnicos":
                        ...
                        if btn_fechar_folha:
                            with st.spinner("📄 Gerando PDF do Demonstrativo..."):
...
2. SOLICITAR COMPRAS: Quando o usuário enviar uma foto, link ou pedir para comprar um item:
   ...
   - Solicite as informações básicas do produto que faltarem:
     ...
     6. Origem da Compra: Identifique se o link/produto é "Nacional" (Mercado Livre, KaBuM, etc) ou "Internacional" (AliExpress, Amazon US, Mouser, DigiKey, etc).
                    ...
                    for tool_call in response_message.tool_calls:
                        ...
                        if fn_name == "calcular_frete_ia":
                            ...
                            resultado = registrar_solicitacao_compra(
                                ...
                                tipo_compra=args.get("tipo_compra", "Nacional"),
                            ...
                            if "sucesso" in resultado:
                                ...
                                tipo_c_txt = args.get("tipo_compra", "Nacional")
                                badge_orig_chat = "🇧🇷 Nacional" if tipo_c_txt == "Nacional" else "✈️ Internacional"
                                ...
                                card_html_gerado = (
                                    ...
                                    f'<div><b>🌐 Origem:</b> {badge_orig_chat}</div>'
...
if aba_gestao:
    with aba_gestao:
        if supabase:
            st.markdown("#### 📊 Painel Consolidado de Compras & Metas")
            ...
            # --- RENDERIZAÇÃO DO GRÁFICO INTERATIVO NA TELA (PLOTLY) ---
            ano_ref_calc = str(datetime.datetime.now().year) if ano_filtro == "Todos" else str(ano_filtro)
            resumo_ui = {m: {"media": 0.0, "meta": 0.0, "gasto": 0.0, "economia": 0.0} for m in range(1, 13)}
            meses_fechados_ui = set()

            try:
                res_f_ui = supabase.table("fechamento_mensal").select("*").eq("ano", ano_ref_calc).execute().data
                if res_f_ui:
                    for f in res_f_ui:
                        m_i = int(str(f.get("mes")).strip())
                        resumo_ui[m_i]["media"] = float(f.get("total_medias") or 0)
                        resumo_ui[m_i]["meta"] = float(f.get("meta_gasto") or 0)
                        resumo_ui[m_i]["gasto"] = float(f.get("gasto_real") or 0)
                        resumo_ui[m_i]["economia"] = float(f.get("economia_total") or 0)
                        meses_fechados_ui.add(m_i)
            except Exception:
                pass

            try:
                resp_cot_ui = supabase.table("cotacoes").select("*").execute().data or []
                for item in resp_cot_ui:
                    dt_c = item.get("data_cotacao") or ""
                    if dt_c.startswith(ano_ref_calc):
                        try:
                            m_i = datetime.datetime.strptime(dt_c[:10], "%Y-%m-%d").month
                            if m_i not in meses_fechados_ui:
                                resumo_ui[m_i]["media"] += float(item.get("media_orcam", 0) or 0)
                                resumo_ui[m_i]["meta"] += float(item.get("preco_alvo", 0) or 0)
                                resumo_ui[m_i]["gasto"] += float(item.get("valor_comprado", 0) or 0)
                                resumo_ui[m_i]["economia"] += float(item.get("economia_real", 0) or 0)
                        except Exception:
                            pass
            except Exception:
                pass

            MESES_NOME_LIST = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
            
            meses_labels = []
            pct_values = []
            for m_i in range(1, 13):
                med_v = resumo_ui[m_i]["media"]
                econ_v = resumo_ui[m_i]["economia"]
                pct_v = (econ_v / med_v * 100) if med_v > 0 else 0.0
                if med_v > 0 or econ_v > 0:
                    meses_labels.append(MESES_NOME_LIST[m_i - 1])
                    pct_values.append(pct_v)
            ...
            if pct_values:
                ...
                fig.add_trace(go.Bar(
                    x=meses_labels,
                    y=pct_values,
                    text=[f"{v:.2f}%".replace(".", ",") for v in pct_values],
                ...
                if len(pct_values) > 1:
                    x_num = np.arange(len(pct_values))
                    coef = np.polyfit(x_num, pct_values, 1)
                ...
                fig.update_layout(
                    title="<b>% ECONOMIA POR MÊS</b>",
                    xaxis_title="Mês",
                    yaxis_title="% Economia",
                    yaxis=dict(ticksuffix="%", range=[0, max(max(pct_values) * 1.25, 20)]),
            ...
            with st.expander("➕ Cadastrar Novo Pedido Manualmente (ADM)"):
                with st.form("form_novo_pedido_adm"):
                    ...
                    with c_f2:
                        ...
                        f_tipo_compra = st.selectbox("Origem da Compra:", ["Nacional", "Internacional"], index=0)
        ...
        if not supabase:
            ...
            with col_f3:
                filtro_origem = st.selectbox(
                    "Filtrar por Origem:",
                    ["Todas as Origens", "Nacional 🇧🇷", "Internacional ✈️"],
                    index=0,
                )
            ...
            if filtro_status == "Selecione para filtrar...":
                ...
                if filtro_origem != "Todas as Origens" and dados_compras:
                    origem_alvo = "Nacional" if "Nacional" in filtro_origem else "Internacional"
                    dados_compras = [
                        item for item in dados_compras
                        if item.get("tipo_compra", "Nacional") == origem_alvo
                    ]
                ...
                if not dados_compras:
                    ...
                    for item in dados_compras:
                        ...
                        if link_cotacao:
                            links_list.append(f'<a href="{link_cotacao}" target="_blank" style="color: #16a34a; text-decoration: none; font-weight: 500;">📊 Cotação</a>')
                        if link_nf:
                            links_list.append(f'<a href="{link_nf}" target="_blank" style="color: #2563eb; text-decoration: none; font-weight: 500;">📄 NF</a>')
                        if link_invoice:
                            links_list.append(f'<a href="{link_invoice}" target="_blank" style="color: #9333ea; text-decoration: none; font-weight: 500;">📄 Invoice</a>')
                        if link_dir:
                            links_list.append(f'<a href="{link_dir}" target="_blank" style="color: #9333ea; text-decoration: none; font-weight: 500;">📄 PDF DIR</a>')
                        ...
                        if codigo_rastreio: detalhes_ped.append(f"<b>Rastreio:</b> {codigo_rastreio}")
                        ...
                        <div style="background: #ffffff; border: 1px solid #e2e8f0; border-left: 4px solid {cor_borda}; padding: 10px 14px; border-radius: 8px; margin-bottom: 6px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
                                <span style="font-weight: 600; font-size: 0.925rem; color: #0f172a;">
                                    📦 {desc} <span style="font-weight: 400; color: #64748b; font-size: 0.825rem;">({qtd} un. - Urgência: {urg_item})</span>
                                    <span style="background: #e2e8f0; color: #334155; padding: 2px 6px; border-radius: 4px; font-size: 0.725rem; font-weight: 600; margin-left: 6px;">{badge_origem}</span>
                                </span>
                        ...
                        with st.container():
                                ...
                                with col_exp1:
                                    with st.expander("📊 Gerar / Gerenciar Cotação"):
                                        with st.form(f"form_gerar_cot_{item_id}"):
                                            default_link_1 = st.session_state.get(f"f1_l_{item_id}") or link if (link and link != "#") else ""
                                            default_nome_1 = st.session_state.get(f"f1_n_{item_id}", extrair_nome_fornecedor(default_link_1))
                                            default_preco_1 = st.session_state.get(f"f1_p_{item_id}", "")
                                            ...
                                            forn1_preco_str = c_f1_2.text_input("Preço 1", value=default_preco_1, key=f"f1_p_{item_id}", placeholder="Preço (15,90)", label_visibility="collapsed")
                                            ...
                                            default_nome_2 = st.session_state.get(f"f2_n_{item_id}", "")
                                            default_preco_2 = st.session_state.get(f"f2_p_{item_id}", "")
                                            default_link_2 = st.session_state.get(f"f2_l_{item_id}", "")

                                            st.caption("🏢 Fornecedor 2")
                                            c_f2_1, c_f2_2, c_f2_3 = st.columns([2, 1, 1])
                                            forn2_nome = c_f2_1.text_input("Nome 2", value=default_nome_2, key=f"f2_n_{item_id}", placeholder="Nome 2", label_visibility="collapsed")
                                            forn2_preco_str = c_f2_2.text_input("Preço 2", value=default_preco_2, key=f"f2_p_{item_id}", placeholder="Preço (15,90)", label_visibility="collapsed")
                                            forn2_frete_str = c_f2_3.text_input("Frete 2", key=f"f2_f_{item_id}", placeholder="Frete (0,00)", label_visibility="collapsed")
                                            forn2_link = st.text_input("Link 2", value=default_link_2, key=f"f2_l_{item_id}", placeholder="🔗 Link Cotação 2", label_visibility="collapsed")

                                            default_nome_3 = st.session_state.get(f"f3_n_{item_id}", "")
                                            default_preco_3 = st.session_state.get(f"f3_p_{item_id}", "")
                                            default_link_3 = st.session_state.get(f"f3_l_{item_id}", "")

                                            st.caption("🏢 Fornecedor 3")
                                            c_f3_1, c_f3_2, c_f3_3 = st.columns([2, 1, 1])
                                            forn3_nome = c_f3_1.text_input("Nome 3", value=default_nome_3, key=f"f3_n_{item_id}", placeholder="Nome 3", label_visibility="collapsed")
                                            forn3_preco_str = c_f3_2.text_input("Preço 3", value=default_preco_3, key=f"f3_p_{item_id}", placeholder="Preço (15,90)", label_visibility="collapsed")
                                            forn3_frete_str = c_f3_3.text_input("Frete 3", key=f"f3_f_{item_id}", placeholder="Frete (0,00)", label_visibility="collapsed")
                                            forn3_link = st.text_input("Link 3", value=default_link_3, key=f"f3_l_{item_id}", placeholder="🔗 Link Cotação 3", label_visibility="collapsed")
                                ...
                                with col_exp2:
                                    with st.expander("📝 Dados da Compra & Anexo NF"):
                                        with st.form(f"form_upload_nf_{item_id}"):
                                            ...
                                            is_internacional = (tipo_c_item == "Internacional")
                                            if not is_internacional:
                                                ...
                                                uploaded_nf = None
                                                validar_nf_ia = False
                                                st.info("✈️ Compras internacionais não utilizam Nota Fiscal. Anexe Invoice e DIR na aba 'Importações'.")
                            ...
                            if status_atual in ["Aguardando entrega", "Aguardando NF"]:
                                with st.expander("🏁 Finalizar Entrega / Recebimento (Acesso ADM)"):
                                    with st.form(f"form_fin_adm_{item_id}"):
                                        col_e1, col_e2 = st.columns(2)
                                        with col_e1:
                                            f_dt_entregue = st.date_input("Data Real de Entrega:", value=datetime.date.today(), format="DD/MM/YYYY", key=f"dt_ent_adm_{item_id}")
                                        with col_e2:
                                            f_qualidade = st.selectbox("Qualidade OK (Sem defeitos/avarias)?", ["SIM", "NÃO"], key=f"qual_adm_{item_id}")

                                        btn_confirm_adm = st.form_submit_button("✅ Finalizar e Confirmar Recebimento ADM")
                                        ...
                                        if btn_confirm_adm:
                                            ...
                                            try:
                                                dt_prom_obj = datetime.datetime.strptime(str(data_prometida), "%Y-%m-%d").date()
                                                dt_prom_salvar = dt_prom_obj.isoformat()
                                            except Exception:
                                                dt_prom_obj = f_dt_entregue
                                                dt_prom_salvar = f_dt_entregue.isoformat()
                                            ...
                                            st.success("🏁 Recebimento forçado pelo ADM registrado com sucesso!")
                                            st.rerun()

                        st.markdown("<div style='margin-bottom: 10px;'></div>", unsafe_allow_html=True)

# =============================================================================
# ABA NOVA: CONTROLE INTERNACIONAL (RESTRITO AO ADM)
# =============================================================================
if aba_inter:
    with aba_inter:
        st.subheader("🌍 Controle de Importações & Invoices")
        st.info("Gerencie os despachos aduaneiros, Rastreios, Invoices e PDFs da DIR de compras internacionais.")

        if not supabase:
            st.warning("⚠️ O Supabase não está conectado.")
        else:
            query_inter = supabase.table("solicitacoes_compras").select("*").eq("tipo_compra", "Internacional").order("id", desc=True).execute()
            dados_inter = query_inter.data or []

            if not dados_inter:
                st.info("Nenhuma solicitação internacional pendente ou registrada encontrada.")
            else:
                st.markdown(f"Exibindo **{len(dados_inter)}** importação(ões):")

                for item in dados_inter:
                    item_id = item.get("id")
                    desc = item.get("item_descricao", "Sem descrição")
                    qtd = item.get("quantidade", 1)
                    solic = item.get("solicitante", "N/A")
                    ...
                    cor_borda = "#6366f1" if status_atual == "Pendente" else ("#10b981" if status_atual == "Finalizado" else "#8b5cf6")

                    links_list = []
                    if link and link != "#":
                        links_list.append(f'<a href="{link}" target="_blank" style="color: #2563eb; text-decoration: none; font-weight: 500;">🔗 Produto</a>')
                    if link_invoice:
                        links_list.append(f'<a href="{link_invoice}" target="_blank" style="color: #9333ea; text-decoration: none; font-weight: 500;">📄 Abrir Invoice</a>')
                    if link_dir:
                        links_list.append(f'<a href="{link_dir}" target="_blank" style="color: #9333ea; text-decoration: none; font-weight: 500;">📄 PDF DIR</a>')
                    ...
                    <div style="background: #ffffff; border: 1px solid #e2e8f0; border-left: 4px solid {cor_borda}; padding: 10px 14px; border-radius: 8px; margin-bottom: 6px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
                            <span style="font-weight: 600; font-size: 0.925rem; color: #0f172a;">
                                📦 {desc} <span style="font-weight: 400; color: #64748b; font-size: 0.825rem;">({qtd} un.)</span>
                                <span style="background: #e2e8f0; color: #334155; padding: 2px 6px; border-radius: 4px; font-size: 0.725rem; font-weight: 600; margin-left: 6px;">✈️ Internacional</span>
                            </span>
                            <span style="background: {cor_borda}; color: #ffffff; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: 600;">{status_atual}</span>
                        </div>
                        <div style="font-size: 0.8rem; color: #475569; margin-bottom: 4px;">
                            <b>Solicitante:</b> {solic} &nbsp;|&nbsp; <b>Rastreio:</b> {codigo_rastreio or 'N/A'}
                        </div>
                        <div style="font-size: 0.775rem; color: #64748b; display: flex; justify-content: space-between; align-items: center; background: #f8fafc; padding: 4px 8px; border-radius: 4px;">
                            <div>{docs_inline}</div>
                        </div>
                    ...
                    with st.container():
                        st.markdown(card_inter_html, unsafe_allow_html=True)

                        with st.expander(f"🛂 Invoice, PDF da DIR & Rastreio (ID #{item_id})"):
                            with st.form(f"form_invoice_{item_id}"):
                                f_rastreio_input = st.text_input("Código de Rastreio:", value=codigo_rastreio or "", key=f"rastreio_input_{item_id}")
                                
                                c_inv1, c_inv2 = st.columns(2)
                                with c_inv1:
                                    up_invoice = st.file_uploader("Upload Invoice (PDF):", type=["pdf"], key=f"inv_up_{item_id}")
                                with c_inv2:
                                    up_dir = st.file_uploader("Upload PDF da DIR:", type=["pdf"], key=f"dir_file_up_{item_id}")
                                
                                btn_salvar_dir = st.form_submit_button("💾 Salvar Dados Internacionais")

                                if btn_salvar_dir:
                                    update_payload = {
                                        "codigo_rastreio": f_rastreio_input.strip(),
                                    }
                                    ...
                                    mes_ano = datetime.datetime.now().strftime("%m-%Y")
                                    dia_cot = datetime.datetime.now().strftime("%d-%m-%Y")
                                    pasta_destino = ["Relatórios IA", "Invoices_e_DIRs", mes_ano, dia_cot]

                                    if up_invoice:
                                        bytes_inv = up_invoice.getvalue()
                                        nome_inv = f"Invoice_{item_id}_{desc_limpa}.pdf"
                                        link_inv_drive = salvar_nf_no_drive(bytes_inv, nome_inv, nome_subpasta=pasta_destino)
                                        if link_inv_drive:
                                            update_payload["link_invoice"] = link_inv_drive

                                    if up_dir:
                                        bytes_dir = up_dir.getvalue()
                                        nome_dir = f"DIR_{item_id}_{desc_limpa}.pdf"
                                        link_dir_drive = salvar_nf_no_drive(bytes_dir, nome_dir, nome_subpasta=pasta_destino)
                                        if link_dir_drive:
                                            update_payload["link_dir"] = link_dir_drive

                                    supabase.table("solicitacoes_compras").update(update_payload).eq("id", item_id).execute()
                                    st.success("✅ Invoice, DIR, Rastreio e PDFs salvos com sucesso!")
                                    st.rerun()
...
# =============================================================================
# ABA 4: FERRAMENTAS E MÓDULOS EXTRAS (RESTRITO AO MODO ADM)
# =============================================================================
if aba_ferramentas:
